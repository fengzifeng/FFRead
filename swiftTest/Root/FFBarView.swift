...
class FFBarView: UIView {
    ...
    func createBarView() {
        for i in 0...2 {
            let button: FFVerticalButton = FFVerticalButton.init(frame: CGRect.init(x: CGFloat(i)*btnWidth, y: 0, width: btnWidth, height: self.height - TabDiff))
            button.tag = i + 1;
